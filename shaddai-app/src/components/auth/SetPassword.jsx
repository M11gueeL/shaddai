...
export default function SetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await acceptInvitation(token, form.password);
      setMessage("¡Cuenta activada correctamente! Contraseña establecida.");
      setCountdown(5);
    } catch (err) {
      const errorMsg = err.response?.data?.error || "Error al establecer la contraseña.";
      setError(errorMsg);
      setLoading(false);
    }
  };
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 bg-white relative">
        <div className="w-full max-w-md space-y-6">
          <div className="text-center lg:text-left space-y-1">
            <h2 className="text-3xl font-bold text-gray-900 tracking-tight">Activar Cuenta</h2>
            <p className="text-gray-500 text-base">Define tu contraseña para acceder al sistema.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            >
              {loading ? (
                <span className="flex items-center justify-center">Activando cuenta...</span>
              ) : (
                "Guardar Contraseña y Activar"
              )}
            </button>
          </form>
