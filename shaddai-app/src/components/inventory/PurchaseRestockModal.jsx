...
export default function PurchaseRestockModal({
  ...
  const validate = () => {
    ...
    lines.forEach((line, idx) => {
      const n = idx + 1;
      if (!line.item_id) list.push(`Linea ${n}: selecciona un producto.`);
      if (!line.quantity || Number(line.quantity) <= 0) list.push(`Linea ${n}: la cantidad debe ser mayor a 0.`);
      if (line.unit_cost === '' || Number(line.unit_cost) < 0) list.push(`Linea ${n}: el costo unitario es invalido.`);
      if (!line.batch_number || !line.batch_number.trim()) list.push(`Linea ${n}: el numero de lote es obligatorio.`);
      if (!line.expiration_date) list.push(`Linea ${n}: la fecha de vencimiento es obligatoria.`);
    });
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="rounded-2xl border border-emerald-100 bg-emerald-50/50 p-4">
          ...
          <p className="text-xs text-emerald-700">
            Carga la factura, distribuye los productos por lote y procesa todo en una sola operacion.
          </p>
        </div>
          ...
          <div className="md:col-span-1">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                title="Agregar proveedor rapido"
              >
        ...
        {showQuickSupplier && (
          <div className="rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4">
            <div className="mb-3 text-sm font-semibold text-indigo-900">Nuevo proveedor rapido</div>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <input
                ...
                placeholder="Nombre *"
                className="rounded-xl border-gray-300 px-3 py-2.5 text-sm"
              />
              <input
                type="text"
          ...
          <table className="min-w-[980px] w-full text-sm">
            <thead className="bg-gray-50 text-gray-700">
              <tr>
                ...
                <th className="px-3 py-2 text-left">Numero de Lote *</th>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="rounded-xl bg-gray-50 px-4 py-2 text-right">
            <div className="text-xs font-medium text-gray-500">Gran Total</div>
